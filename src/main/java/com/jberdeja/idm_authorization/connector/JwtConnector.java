...
public class JwtConnector {
    ...
    private String passwordForAuthenticate;
        ...
        public String obtainClaimsFromToken(String pathVariable){
            try{
                JWTAuthenticateRequest body = buildBodyForAuthentication();
                JWTResponse jwtResponse = authenticateForObtainToken(body);
                return getClaimsFromToken(jwtResponse.getJwt(), pathVariable);
            }catch(Exception e){
        ...
        private String getClaimsFromToken(String token, String pathVariable){
            try{
